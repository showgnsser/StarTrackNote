...
信号从强变弱时，监测器先连续发布低SNR/CN0证据；FSM建立LongSlow候选并累计；满足
迟滞后只把LongSlow写成pending；Engine生成下一历元的长周期策略命令；后续PDI回报匹配的
command id后，FSM才提交LongSlow。提交时保留NCO，只重建不兼容观察窗。这个过程正是上图
从Active走到Committed的实例。
