...
| 动态 | MidDynamic、HighDynamic | 残余动态超限补救及退出恢复，不按场景标签直接进入 |

## 11.3 三种入口

| 捕获移交入口 | 首个状态 | 入口含义 |
|---|---|---|
| Strong | ShortFast | 强信号、短周期快速接管 |
| Normal | LongFast | 普通移交、160 ms长统计入口 |
| Aided | OutdoorBitAiding | 显式外部bit辅助入口 |
...
上图下半部分就是长周期链：LongFast负责接管，LongNormal负责过渡，LongSlow负责低动态
稳态，Sensitivity负责自主极弱保持。反向箭头表示信号恢复后的逐级回升，不是任意直跳。
...
动态分类的物理源职责是LongSlow。软件还有一个时间轴压缩边界：ShortSlow低SNR证据已经
准备把策略交给LongSlow时，如果同一批成熟动态证据又明确要求HighDynamic，可以把
“ShortSlow → LongSlow → HighDynamic”折叠成一次等待命令确认的交接，避免换档清窗后丢掉
已成熟动态统计。这不表示ShortSlow平时直接用输入真值判断高动态。
