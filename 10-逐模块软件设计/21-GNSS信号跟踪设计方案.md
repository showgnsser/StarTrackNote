...
五种导频按候选擦二次码，统一20 ms分块相干，再累加功率。E1、B1C分别用5个4 ms、2个10 ms主码结果组成一块；1／2／4 s窗分别累加50／100／200块，B1C的18 s窗累加900块。B1C按已知码序列复用两主码的和、差功率，直接比较二次码起点。
...
数据型与E1共用扩窗规则：1 s搜索失败后清窗，用新的2 s重试，再失败用新的4 s；达到4 s后继续取独立新窗。搜索通过后，首次确认窗取搜索窗的两倍、最多4 s。2 s确认质量失败时保留候选，用新4 s确认；4 s仍失败则丢弃候选、继续4 s搜索。质量通过但边界不同，则保存新候选并再次独立确认。

例如E1用1 s搜索失败、2 s搜索通过，再用4 s新数据确认，搜索与确认共7 s。每个相干块仍为20 ms，延长的是跨块功率累计时间，不是连续相干时长；搜索与确认之间不复用数据。

L5、B2a、E5a仍使用1 s搜索与1 s确认，失败搜索新增20 ms滑动重试；B1C使用18 s搜索与18 s确认，失败搜索新增1 s。四种导频确认质量失败时丢弃候选重搜。所有信号的同步重试均不改变载码环更新节拍，并受第4章总时限约束；E1仍从捕获移交起计20 s。
...
下表每个信号、每个功率点32例，均为300 s低动态保持，包含静态与恒定多普勒斜率。E1使用扩窗后的同输入复验结果，其余七信号保留载噪比入口实验8645374的结果。表内为持续保持数；“含码精度”另见实验逐例记录。
...
以8645374为基线，比较固定1 s搜索／1 s确认、只扩搜索、只延长确认及共用数据型扩窗规则。目标牵引两批共1944例，强入口总32／35 dB-Hz，中弱入口总27 dB-Hz，覆盖±62.5 Hz以内的非零频差、±0.35 chip以内码差及静态、±1 Hz/s。
...
后两行是首行的分组，不能重复合计。完整稳定包括正确同步、90 s内无退出以及末10 s频差RMS＜5 Hz、码差RMS＜0.20 chip。强入口的完整稳定数不变；同步中位时间约增加1 s。扩窗改善主要在大码差下的同步后保持，不代表正确同步概率提高。

512例纯噪声隔离测试的误锁由1例降为0例；384例固定残差测试的误锁由2例降为0例，但正确同步也由285降为258。零误锁是有限样本结果，扩窗带来的保守判决和超时代价同时保留。

E1后级736例持续保持702→701，双动态317/320、功率边界96/96不变；总10 dB-Hz的300 s子组30/32→27/32，已更新8.3。其他七信号343组完整输出不变。规则与逐例结果见[E1同步扩窗实验](实验记录-20260917-E1同步扩窗.md)。

### 8.5 证据边界

既有牵引矩阵在c7a1504基线稳定通过15,140/15,264，正确同步15,210/15,264；E1中弱入口±0.35 chip仍有明显边界失败。该牵引矩阵是历史证据，本次未完整重跑，不能与载噪比入口的6,192组保持对照混算；E1扩窗后的局部复验另见8.4。BOC按±0.25 chip移交设计，±0.35 chip作为扩展压力，不据此承诺±0.50 chip直接牵引能力。
